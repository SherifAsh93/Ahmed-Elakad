...
// Body: { section, year, collectionId, images, coverIndex }
export async function POST(req: NextRequest) {
  if (!(await checkAuth())) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const { section, year, collectionId, images, coverIndex } = await req.json();

  if (!section || !year || !collectionId || !Array.isArray(images)) {
  ...
  coll.images = images;
  if (typeof coverIndex === "number") coll.coverIndex = coverIndex;
