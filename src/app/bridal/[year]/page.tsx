...
export default async function BridalYearPage({
  ...
  // Only show tabs for years that actually have published collections,
  // so the tab bar doesn't fill up with dead years — but keep the current
  // year visible even if it's still empty, since that's what's being viewed.
  const visibleYears = ALL_YEARS.filter(
    (y) => y === year || (years[y]?.collections ?? []).some((c) => c.images.length > 0)
  );
      ...
      <div className="bg-[#f9f7f4]">
        ...
        {year === "all" ? (
              ...
              allYearGroups.map(({ year: y, collections: colls }) => (
                <section key={y}>
                  ...
                  <div className="px-4 sm:px-8 md:px-12 max-w-screen-xl mx-auto">
                    <CollectionGrid collections={colls} section="bridal" year={y} />
                  </div>
                </section>
              ))
            ) : (
          ...
          <div className="px-4 sm:px-8 md:px-12 pb-20 max-w-screen-xl mx-auto">
            <CollectionGrid collections={collections} section="bridal" year={year} />
          </div>
        )}
      </div>
    </main>
